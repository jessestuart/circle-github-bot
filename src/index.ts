...
const REQUIRED_ENV: Array<keyof IEnvironment> = [
  'buildUrl',
  'home',
  'repo',
  'sha1',
...
export default class Bot {
  public static create(options: IOptions = {}) {
    ...
    const env: IEnvironment = {
      ...
      commitMessage: exec('git --no-pager log --pretty=format:"%s" -1').replace(
        /\\"/g,
        '\\\\"'
      ),
      githubDomain,
  ...
  public artifactUrl(artifactPath: string) {
    return `${this.env.buildUrl}/artifacts/0/${
      this.env.home
  ...
  public artifactLink(artifactPath: string, text: string) {
    return `<a href='${this.artifactUrl(
      artifactPath
